...
const DoubtAccept = () => {
  ...
  const handleInfo = async(studentId,studName)=>{
    try {
      const response = await axios.post(`/info-post`, {
        myId: teacherId,
        senderId: studentId,
        senderName: studName
      });
      const response2 = await axios.post(`/info-post`, {
        myId: studentId,
        senderId:teacherId ,
        senderName: name
      });
  ...
  const handleAcceptDoubt = async (doubtId ,studentId,studName) => {
    try {
      await axios.put(`/teacher/accepted`,{doubtId});
  ...
  const getFilteredDoubts = async () => {
    try {
      const url = `/teacher/get-doubts/${teacherId}`;
